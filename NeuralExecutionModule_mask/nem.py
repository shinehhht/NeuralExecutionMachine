...
from NeuralExecutionModule_mask.crossAttentionGrouped import Prog2Transformer, InitialParseGrouped
from NeuralExecutionModule_mask.registers import Registers
from NeuralExecutionModule_mask.featurePproj import FeatureProj,FeatureProjWithRegisters
from NeuralExecutionModule_mask.interpreter import Interpreter,InterpreterWithRegisters, InterpreterWithRegistersAndKbit
...
class GateProj(nn.Module):
    def __init__(self,config):
        super().__init__()
        self.config = config
        self.warmup_steps = getattr(config, 'warmup_steps', 100)
...
class NeuralExecutionModuleWithRegistersAndKbit(nn.Module):
    ...
    def forward(self, hidden_states, steps, problem_mask):
        ...
        fused_hidden_states = self.Fuse2Main(hidden_states, gated_updated_registers, mask_expanded)
