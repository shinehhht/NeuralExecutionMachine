...
class GroupedCrossAttention(nn.Module):
    ...
    def multihead_cross_attn(self, query, key, value, key_mask=None,query_mask=None):
        ...
        if key_mask is not None:
            ...
            attn_scores = attn_scores.masked_fill(mask == 2, float('-inf'))
...
class Prog2Transformer(nn.Module):
    def __init__(self,config, groups=4):
        super().__init__()
        
        self.o_proj = GroupedLinear(config.nem_dim, config.hidden_dim, groups=groups)
        nn.init.zeros_(self.o_proj.weight)
        if self.o_proj.bias is not None:
            nn.init.zeros_(self.o_proj.bias)
            
    def forward(self, registers):
        return self.o_proj(registers)
...
class InitialParseGrouped(GroupedCrossAttention):
    def __init__(self, config, groups=4):
        ...
        total_slots_with_registers = 2 + 5 * config.num_instructions + config.n_regs
    ...
    def forward(self, hidden_state, mask):
        ...
        value_initial = Z[:, 0:1, :]
        k_initial = Z[:, 1:2, :]
        final_registers_gate = Z[:, 2:2+self.config.n_regs, :]
